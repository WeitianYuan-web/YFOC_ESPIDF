...
esp_err_t svpwm_new_inverter(const inverter_config_t *config, inverter_handle_t *ret_inverter)
{
    ...
    for (int i = 0; i < 3; i++) {
        // 只创建第一个发生器（高边PWM）
        if (config->gen_gpios[i][0] >= 0) {
            gen_config.gen_gpio_num = config->gen_gpios[i][0];
            ESP_GOTO_ON_ERROR(mcpwm_new_generator(svpwm_dev->operators[i], &gen_config, &svpwm_dev->generators[i][0]), 
                             err, TAG, "Create PWM generator pin %d failed", gen_config.gen_gpio_num);
        }
        // 如果指定了第二个发生器（低边PWM）则创建
        if (config->gen_gpios[i][1] >= 0) {
            gen_config.gen_gpio_num = config->gen_gpios[i][1];
            ESP_GOTO_ON_ERROR(mcpwm_new_generator(svpwm_dev->operators[i], &gen_config, &svpwm_dev->generators[i][1]), 
                             err, TAG, "Create PWM generator pin %d failed", gen_config.gen_gpio_num);
        }
    }

    for (int i = 0; i < 3; i++) {
        if (svpwm_dev->generators[i][0]) {
    ...
    // 只有当配置了死区相关参数和第二个发生器时才配置死区时间
    if (config->dt_config.posedge_delay_ticks > 0 || config->inv_dt_config.negedge_delay_ticks > 0) {
        for (int i = 0; i < 3; i++) {
            if (svpwm_dev->generators[i][0] && svpwm_dev->generators[i][1]) {
                ESP_GOTO_ON_ERROR(mcpwm_generator_set_dead_time(svpwm_dev->generators[i][0], svpwm_dev->generators[i][0], &config->dt_config), 
                                 err, TAG, "Setup deadtime failed");
                ESP_GOTO_ON_ERROR(mcpwm_generator_set_dead_time(svpwm_dev->generators[i][0], svpwm_dev->generators[i][1], &config->inv_dt_config), 
