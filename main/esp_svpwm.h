...
typedef struct inverter_config {
    ...
    int gen_gpios[3][2];                        // GPIO pins for generator config, -1 means not used
    mcpwm_dead_time_config_t dt_config;         // dead time config for positive pwm output (optional)
    mcpwm_dead_time_config_t inv_dt_config;     // dead time config for negative pwm output (optional)
} inverter_config_t;
